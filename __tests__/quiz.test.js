...
describe('CRUD for courses by teacher', () => {
   ...
   test('update method for quiz by id', async () => {
    const response = await request.put(`/quiz/student/${id}`)
        .send({
            solution:[{"email":"test","solution":"after111122222222"}]
        })
        .set({ "Authorization": `Bearer ${studentToken}` });
    expect(response.status).toBe(200);
    expect(response.body.solution).toEqual([{"email":"test","solution":"after111122222222"}]);
})
